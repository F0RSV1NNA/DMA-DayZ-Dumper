...
{
	if (!signature || !mask || range_start >= range_end)
		return 0;
	...
	for (size_t i = 0; i < buffer.size() - patternLength; ++i)
	{
		bool found = true;
		for (size_t j = 0; j < patternLength; ++j)
		{
			if (mask[j] == 'x' && buffer[i + j] != (uint8_t)signature[j])
			{
				found = false;
				break;
			}
...
// Add the other resolve methods similarly...
